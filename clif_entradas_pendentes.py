...
def open_link3(login, senha, datainicio, datafim, time_interval, armazem_index):
    ...
    try:
        ...
        while True:  # Loop infinito
            ...
            driver.find_element(By.XPATH, '//*[@id="filtro_unidade"]').click()
            time.sleep(2)
            driver.find_element(By.XPATH, '//*[@id="filtro_unidade"]/option[2]').click()
            time.sleep(2)
            driver.find_element(By.XPATH, '//*[@id="RelentradaspendenteTipofiltro"]/option[2]').click()
            time.sleep(2)
            driver.find_element(By.XPATH, '//*[@id="FormRelatorio"]/div[8]/div/input').click()

            # Nome do arquivo que será baixado
            filename = os.path.join(arquivo_data, 'relatorio_entradas_pendentes_col.xlsx')  # Ajuste o nome e extensão conforme necessário
