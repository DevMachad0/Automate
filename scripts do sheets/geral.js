function buscarArquivo2() {
    var nomeArquivo = "relatorio_saidas_geral_col.xlsx"; // Nome do arquivo a ser buscado
    var sheet = SpreadsheetApp.openById("1cS2QSsBlSfctV1jLx9oCiH4FvRVxVVtzqa4YQ3mF8Yw").getSheetByName("relatorio_geral");
  
    // Buscar o arquivo com base no nome
    var resultado = buscarArquivoPorNome2(nomeArquivo);
    ...
    if (resultado[0][2]) { // Verifica se o arquivo foi encontrado
      ...
      if (novoId) {
        ...
        var dados = novaPlanilha.getSheetByName("relsaidasgeral_col").getRange("A1:Z19999").getValues();
  ...
  function buscarArquivoPorNome2(nomeArquivo) {
    if (!nomeArquivo) {
