function buscarArquivo3() {
    var nomeArquivo = "relatorio_entradas_pendentes_col.xlsx"; // Nome do arquivo a ser buscado
    var sheet = SpreadsheetApp.openById("1cS2QSsBlSfctV1jLx9oCiH4FvRVxVVtzqa4YQ3mF8Yw").getSheetByName("entradas_pendentes");
  
    // Buscar o arquivo com base no nome
    var resultado = buscarArquivoPorNome3(nomeArquivo);
    ...
    if (resultado[0][2]) { // Verifica se o arquivo foi encontrado
      ...
      if (novoId) {
        ...
        var dados = novaPlanilha.getSheetByName("relentradaspendentes_col").getRange("A1:Z19999").getValues();
  
        // **Agora faz a limpeza somente antes de colar os dados**
        sheet.getRange(1, 1, 9999, 26).clearContent();
  ...
  function buscarArquivoPorNome3(nomeArquivo) {
    if (!nomeArquivo) {
    ...
    if (files.hasNext()) {
      var file = files.next();
      results.push([file.getName(), file.getUrl(), file]);
